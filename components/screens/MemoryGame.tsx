...
const MemoryGame: React.FC = ({ navigation: { navigate } }: Props) => {
  ...
  async function playSound(respuesta: String) {
    ...
    const { sound: audioSound } = await Audio.Sound.createAsync(
      respuesta === "correcta"
        ? require("./../../assets/audio/correct-ding.mp3")
        : require("./../../assets/audio/error-fallo-1.mp3")
    );
    setSound(audioSound);
  ...
  useEffect(() => {
    ...
    const randomImageIndex = Math.floor(
      Math.random() * nivelesCat[dropdownValue1][bandera].length
    );
    const numrandom = Math.floor(
      Math.random() *
        nivelesCat[dropdownValue1][bandera][randomImageIndex].imagen.length
    );
    const randomImagePathJson =
      nivelesCat[dropdownValue1][bandera][randomImageIndex].imagen[numrandom];
    const randomImagePath =
      imagePathSet[parseInt(randomImagePathJson.replace(/[^\d]/g, "")) - 1];
    ...
    const numero1 = setTimeout(() => {
      const newnumrandom = Math.floor(
        Math.random() *
          nivelesCat[dropdownValue1][bandera][randomImageIndex].imagen.length
      );
      const newrandomImagePathJson =
        nivelesCat[dropdownValue1][bandera][randomImageIndex].imagen[
          newnumrandom
        ];
      const newrandomImagePath =
        imagePathSet[
          parseInt(newrandomImagePathJson.replace(/[^\d]/g, "")) - 1
        ];
          ...
          <View>
            ...
            {previousImage !== null && (
              ...
              >
                <TouchableOpacity
                  onPress={() =>
                    handleOptionSelected(currentImage === previousImage)
                  }
                  style={{
